...
DreameClaw Crew 是一个开源的多智能体协作平台。不同于单一 Agent 工具，DreameClaw Crew 赋予每个 AI Agent **持久身份**、**长期记忆**和**独立工作空间**——让它们组成一个团队协作工作，也和你一起工作。

## 🌟 DreameClaw Crew 的独特之处
...
### 🏢 数字员工，而非聊天机器人
DreameClaw Crew 的 Agent 是**组织的数字员工**。每个 Agent 了解完整的组织架构、可以发消息、委派任务、建立工作关系——就像一位新员工融入团队。
...
> **说明：** DreameClaw Crew 不在本地运行任何 AI 模型——所有 LLM 推理均由外部 API 提供商处理（OpenAI、Anthropic 等）。本地部署本质上是一个标准 Web 应用 + Docker 编排。
...
| 方案 | 命令 |
|---|---|
| **浅克隆**（仅下载最新提交） | `git clone --depth 1 https://github.com/dataelement/DreameClaw Crew.git` |
| **下载 Release 压缩包**（无需 git） | 前往 [Releases](https://github.com/dataelement/DreameClaw Crew/releases) 下载 `.tar.gz` |
...
git clone https://ghfast.top/https://github.com/dataelement/DreameClaw Crew.git
git clone https://ghproxy.com/https://github.com/dataelement/DreameClaw Crew.git
git clone https://gitclone.com/github.com/dataelement/DreameClaw Crew.git
```
...
欢迎各种形式的贡献！无论是修复 Bug、添加功能、改进文档还是翻译——请查看我们的[贡献指南](CONTRIBUTING.md)开始参与。新手可以关注 [`good first issue`](https://github.com/dataelement/DreameClaw Crew/labels/good%20first%20issue) 标签。
