...
async def _send_to_agent_background(
    ...
    try:
        ...
        async with async_session() as db:
            ...
            system_prompt = await build_agent_context(target_agent_id, target_agent_name, target_role_description)
...
async def send_message(
    ...
    if target_member.feishu_open_id and (not channel_hint or channel_hint == "feishu"):
        ...
        if resp.get("code") == 0:
            ...
            raise HTTPException(
                status_code=502, detail=f"Feishu send failed: {resp.get('msg')} (code {resp.get('code')})"
            )
